...
static void test_matrix(GTestStats* stats) {
    ...
    EXPECT_TRUE(stats, is_eq(m, 1, 0,  2.5,
                                0, 1, -4  ));
    m = GMatrix::Scale(2.5, -4);
    EXPECT_TRUE(stats, is_eq(m, 2.5, 0, 0,
                                0,  -4, 0));
    m = GMatrix::Rotate(M_PI/2);
    EXPECT_TRUE(stats, is_eq(m, 0, -1, 0,
                                1,  0, 0));
    m = GMatrix::Rotate(M_PI);
    EXPECT_TRUE(stats, is_eq(m, -1,  0, 0,
                                 0, -1, 0));
    m = GMatrix::Rotate(M_PI/4);
    const float r2 = sqrt(2.0)/2;
    ...
    m3 = m * m2;
    EXPECT_TRUE(stats, is_eq(m3, -2,  0, 0,
                                  0, -6, 0));
    m2 = GMatrix::Translate(5, 6);
    m3 = m2 * m;
    EXPECT_TRUE(stats, is_eq(m3, 2, 0, 5,
                                 0, 3, 6));
}
...
static void test_matrix_inv(GTestStats* stats) {
    ...
    EXPECT_TRUE(stats, m.invert(&m2) && is_eq(m2, 2, 0,    0,
                                                  0, 0.25, 0));
    // check for alias detection of src and dst
    EXPECT_TRUE(stats, m.invert(&m)  && is_eq(m,  2, 0,    0,
                                                  0, 0.25, 0));
